...
if ! tmux has-session -t $SESSION 2>/dev/null; then
  ...
  # Pane 2 (bottom-left): ROS2 listener
  ...
  tmux send-keys -t $SESSION:0.2 "ros2 run demo_nodes_cpp listener" C-m
  ...
  tmux send-keys -t $SESSION:0.3 "ros2 run ros1_bridge dynamic_bridge --bridge-all-topics --ros-args -p ignore_topics:="['/rosout']"" C-m
fi
