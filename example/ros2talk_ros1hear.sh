...
if ! tmux has-session -t $SESSION 2>/dev/null; then
  ...
  # Send your ROS commands into each pane: "roscore --master-logger-level=fatal"
  ...
  # Pane 1 (top-right): ROS2 talker
  tmux send-keys -t $SESSION:0.1 "source \$ROS2_INSTALL_PATH/setup.bash" C-m
  tmux send-keys -t $SESSION:0.1 "ros2 run demo_nodes_py talker --disable-rosout-logs" C-m

  # Pane 2 (bottom-left): ROS1 listener
  tmux send-keys -t $SESSION:0.2 "source \$ROS1_INSTALL_PATH/setup.bash" C-m
  tmux send-keys -t $SESSION:0.2 "rosrun roscpp_tutorials listener" C-m
  ...
  tmux send-keys -t $SESSION:0.3 "ros2 run ros1_bridge dynamic_bridge --disable-rosout-logs" C-m
fi
